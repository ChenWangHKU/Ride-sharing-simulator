...
class ControlCenter:
    ...
    (1) REQUEST
        0.  Service rate
        1.  Service rate (non-ride-pooling)
        2.  Service rate (ride-pooling)
        3.  The average assigning time
        4.  The average waiting time of requests
        5.  The average detour time
        6.  The average detour time ratio
        7.  The average total time ratio
        8.  The average detour distance
        9.  The average detour distance ratio
        10. Cancellation rate (assign)
        11. Cancallation rate (pickup)
        
    (2) VEHICLE
        1. The number of vehicles
        2. The average idle time
        3. The average income of all vehicles
        4. The average travel distance of all vehicles
    
    (3) Key measurement metrics
        1. Delivery efficiency factor (DEF)
        2. Passenger emission factor (PEF) * 4
        3. Delay factor (DF)
    '''
    def CalculateResults(self):
        requests_results = np.zeros((12))
        ...
        for requests in self.requests_all:
            for request in requests:
                ...
                if request.finish_dropoff:
                    PMD += request.original_travel_distance
                    requests_results[0] += 1
                    if request.max_tol_num_person == 1:
                        requests_results[1] += 1
                    else:
                        requests_results[2] += 1
                    requests_results[3] += request.assign_timepoint - request.send_request_timepoint
                    requests_results[4] += request.pickup_timepoint - request.assign_timepoint
                    requests_results[5] += max(0, request.time_on_vehicle - request.original_travel_time)
                    requests_results[6] += max(0, request.time_on_vehicle - request.original_travel_time) / request.original_travel_time
                    requests_results[7] += (request.time_on_vehicle + request.pickup_timepoint - request.send_request_timepoint) / request.original_travel_time
                    requests_results[8] += max(0, request.distance_on_vehicle - request.original_travel_distance)
                    requests_results[9] += max(0, request.distance_on_vehicle - request.original_travel_distance) / request.original_travel_distance
                ...
                # Note: Here, we assume that there is no passenger at any vehicles. In other words, all trips are finished at the end of simulation
                    ...
                    if request.finish_assign:
                        ...
                        requests_results[11] += 1
        
        # mean value
        if num_req_non > 0:
            requests_results[1] /= num_req_non
        if num_req_pool > 0:
            requests_results[2] /= num_req_pool
        requests_results[3:10] /= requests_results[0]
        requests_results[10:] /= num_req
        requests_results[0] /= num_req
        ...
        VMT = vehicles_results[3]
        vehicles_results[1:] /= vehicles_results[0]


        # Measurement metrics
        metrics[0] = VMT / PMD
        metrics[1:5] = self.action_system.E / (PMD / 1000)
        metrics[5] = np.mean(np.array(self.environment.DF))
        metrics[6] = np.mean(np.array(self.environment.SPEED))
        
        # Initialize the metrics
        self.action_system.E = np.zeros(4)
        self.environment.DF = []
        self.environment.SPEED = []
        print('*'*30)
        print('Time for heuristic algorithm: ', self.RTV_system.t)
        print('Time for complete search: ', self.RTV_system.t2)
        print(self.RTV_system.matrix)
        print(self.RTV_system.consistency)
        print('*'*30)

        return requests_results, vehicles_results, metrics
