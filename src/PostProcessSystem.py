...
class PostProcessSystem:
    ...
    def ReqTimeSta(self, ax, requests, HIST = False):
        if HIST:
            time = []
            ...
            plt.hist(time, bins = tmax-tmin, range = [tmin, tmax], density = False, facecolor = 'blue', edgecolor = 'blue', alpha = 0.5)
        else:
            hours = int((self.environment.end_time - self.environment.start_time) / 3600)
            time = np.linspace(self.environment.start_time/3600, self.environment.end_time/3600-1, hours)
            num = np.zeros(hours)
            for reqs in requests:
                for req in reqs:
                    num[int(req.send_request_timepoint / 3600)] += 1
            plt.plot(time, num, 'c-^', linewidth = 5, markersize = 15)
        plt.xlabel('Time (h)', fontsize = self.label_fs, fontproperties = self.fonttype)
        plt.ylabel('Number of requests', fontsize = self.label_fs, fontproperties = self.fonttype)
        ...
        ax.set_xlim(self.environment.start_time/3600-0.5, self.environment.end_time/3600-0.5)
    ...
    def ReqDisSta(self, ax, requests, MaxDis = None, nor_fit = True):
        ...
        if nor_fit:
            ...
            # plot
            plt.plot(bins, y, 'r--')
            plt.ylabel('Probability Density', fontsize = self.label_fs, fontproperties = self.fonttype)
            plt.title('Histogram of Normal Distribution: $\mu = %.3f$, $\sigma=%.3f$'%(miu,sigma), fontsize = self.title_fs, fontproperties = self.fonttype)
        else:
            plt.hist(dis, bins = 100, density = False, facecolor = 'blue', alpha = 0.5, rwidth=0.85) 
            plt.ylabel('Number of requests', fontsize = self.label_fs, fontproperties = self.fonttype)
        plt.xlabel('Distance (km)', fontsize = self.label_fs, fontproperties = self.fonttype)
        plt.xticks(fontsize = self.tick_fs, fontproperties = self.fonttype)
        plt.yticks(fontsize = self.tick_fs, fontproperties = self.fonttype)
    ...
    def DrawRequests(self, ax, requests, type = 'pickup', s = 10, count = False, cmap = 'viridis', cmax = 10, color = 'red', draw_grid = False):
        ...
        if count:          
            ...
            cb = plt.colorbar()
            cb.ax.tick_params(labelsize = self.tick_fs)
            cb.set_label('Number of requests', fontdict = self.cb_font)
            # ticks of colorbar
            ticks = [1]
