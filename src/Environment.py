...
class ENVIRONMENT:
    def __init__(self,
        ...
        # congestion measurement: average delay factor
        self.max_nv = 1200
        self.DF = []
        self.SPEED = []
    ...
    # function: Calculate the average delay factor (DF) of all roads   
    # params: None
    # return: Average delay factor
    # Note: DF = free-flow speed / congested speed
    def CalDelayFactor(self):
        DF = 0
        expand = self.road_extend
        num = 0
        for road in self.roads:
            ...
            # We only consider the road network in the area of interest
            if (lng_u >= self.lng_min - expand and lng_u <= self.lng_max + expand and lat_u >= self.lat_min - expand and lat_u <= self.lat_max + expand
                and lng_v >= self.lng_min - expand and lng_v <= self.lng_max + expand and lat_v >= self.lat_min - expand and lat_v <= self.lat_max + expand) and road.DF > road.DF_min + 0.01:
                    DF += road.DF
                    num += 1   
        if DF > 0:
            DF /= num
            DF = DF * len(self.vehicles) / self.max_nv + road.DF_min * (1 - len(self.vehicles) / self.max_nv)
        else:
            DF = road.DF_min
        self.DF.append(DF)
