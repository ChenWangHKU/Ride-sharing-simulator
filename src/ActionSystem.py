...
class ActionSystem:
    def __init__(self,
        ...
        # Emission measurement: total emission
        '''
            Vehicles can only emitte pollutants if they take actions, thus we calculate the emissions in this Action System.
        '''
        self.E = np.zeros(4)
        self.params = np.array([[3.32e-1, -1.76e1, 1.45e3, 0, 8.0e-4, 9.13E-2, 3.51],
                                [0, -3.4e-2, 5.11, 0, 1.87e-3, -5.29e-1, 3.75e1],
                                [3.86e-5, -8.58e-3, 5.77e-1, 0, 0, 0, 5.43],
                                [3.55e-6, -1.39e-4, 4.74e-2, 0, 0, 0, 4.01]])
    ...
    def UpdateVehicles(self, final_trips, final_paths, vehicles = None):
        ...
        if self.current_timepoint <= self.cfg.SIMULATION.END:
            self.environment.CalDelayFactor()
    ...
    def SimulateVehicleActions(self, vehicles = None, ERR_THRE = 10):
        ...
        for vehicle in vehicles:
            ...
            if vehicle.dis_from_cur_pos >= vehicle.dis_to_next_pos or vehicle.dis_to_next_pos - vehicle.dis_from_cur_pos < ERR_THRE:
                '''
                    We calculate the emissions once the vehicle arrives at the end of a road. The average speed can be calculated as: v = dis / remaining_time.
                '''
                speed = (vehicle.dis_to_next_pos / vehicle.remaining_time_for_current_node) * 3.6 # unit: km/h
                
                for i in range(len(self.E)):
                    EF = (self.params[i, 0]*speed**2 + self.params[i, 1]*speed + self.params[i, 2]) / (self.params[i, 4]*speed**2 + self.params[i, 5]*speed + self.params[i,6])
                    emissions = vehicle.dis_to_next_pos / 1000 * EF
                    self.E[i] += emissions
                    
                # Accumulate the distance travelled of the vehicle
                vehicle.total_distance += vehicle.dis_to_next_pos
