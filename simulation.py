...
def main():
    ...
    # Record the results
    def LogResults(logger, requests_results, vehicles_results, metrics):
        # Requests
        logger.info('Service rate:                     {}'.format(requests_results[0]))
        logger.info('Service rate (non-ride-pooling):  {}'.format(requests_results[1]))
        logger.info('Service rate (ride-pooling):      {}'.format(requests_results[2]))
        logger.info('The average assigning time (s):   {}'.format(requests_results[3]))
        logger.info('The average pick-up time (min):   {}'.format(requests_results[4] / 60))
        logger.info('The average detour time (min):    {}'.format(requests_results[5] / 60))
        logger.info('The average detour time ratio:    {}'.format(requests_results[6]))
        logger.info('The average total time ratio:     {}'.format(requests_results[7]))
        logger.info('The average detour distance (km): {}'.format(requests_results[8] / 1000))
        logger.info('The average detour distance ratio:{}'.format(requests_results[9]))
        logger.info('Cancellation rate (pickup):       {}'.format(requests_results[10]))
        logger.info('Cancellation rate (assign):       {}'.format(requests_results[11]))
        logger.info('******************************')
        
        # Vehicles
        logger.info('The average idle time of all vehicles (min):      {}'.format(vehicles_results[1] / 60))
        logger.info('The average income of all vehicles (RMB):         {}'.format(vehicles_results[2]))
        logger.info('The average travel distance of all vehicles (km): {}'.format(vehicles_results[3] / 1000))
        logger.info('******************************')

        # metrics
        logger.info('Delivery efficiency factor (DEF):               {}'.format(metrics[0]))
        logger.info('Passenger emission factor (PEF) [CO2 (g/km)]:   {}'.format(metrics[1]))
        logger.info('Passenger emission factor (PEF) [CO  (g/km)]:   {}'.format(metrics[2]))
        logger.info('Passenger emission factor (PEF) [NOx (g/km)]:   {}'.format(metrics[3]))
        logger.info('Passenger emission factor (PEF) [HC  (g/km)]:   {}'.format(metrics[4]))
        logger.info('Delay factor (DF):                              {}'.format(metrics[5]))
        logger.info('Average speed of vehicles (m/s):                {}'.format(metrics[6]))
        logger.info('******************************')
    ...
    for pooling_rate in pooling_rates:
        for veh_num in cfg.VEHICLE.NUM:
        # Load requests for test
            ...
            lam = req_num / (end_timepoint - start_timepoint)
            x = avg_trip_dis * lam / velocity / veh_num
            u = (cfg.REQUEST.CONSTRAINTS.max_travel_time_mul - 1 + float(cfg.VEHICLE.MAXCAPACITY)**0.33)*x
            logger.info('******************************')
            logger.info('The number of vehicles (N):   {}'.format(veh_num))
            logger.info('Request rate (lambda):        {}'.format(lam))
            logger.info('Average trip distance (d):    {}'.format(avg_trip_dis))
            logger.info('Average vehicle velocity (v): {}'.format(velocity))
            logger.info('Normalized load (x):          {}'.format(x))
            logger.info('System load (u):              {}'.format(u))
            logger.info('******************************')
            ...
            if args.DrawDistribution:
                ...
                plt.savefig(os.path.join(args.OutputDir, cfg_file_name, 'ReqTimeDist.pdf'))
                plt.close('all')
                ...
                plt.subplots_adjust(left=0.15, right=0.98)
                plt.savefig(os.path.join(args.OutputDir, cfg_file_name, 'ReqDisDist.pdf'))
                plt.close('all')
                logger.info('******************************')
                ...
                ax = control_center.DrawRequests(ax, requests, type = 'pickup', s = 15, count = True, cmap = 'Reds', cmax = 100, color = 'red', draw_grid = False)
                plt.subplots_adjust(left=0.15)
                plt.savefig(os.path.join(args.OutputDir, cfg_file_name, 'requests.pdf'))
                plt.close('all')
                ...
                plt.savefig(os.path.join(args.OutputDir, cfg_file_name, 'vehicles.pdf'))
                plt.close('all')
            ...
            for i in range(epoch_num):
                ...
                req_num_avg += req_num
                # Record the results
                requests_results, vehicles_results, metrics = control_center.CalculateResults()
                ...
                LogResults(logger, requests_results, vehicles_results, metrics)
            ...
            # metrics_all = np.array(metrics_all).mean(axis = 0)
            # logger.info('****************** Simulation Average Results *********************')
            # LogResults(logger, requests_results_all, vehicles_results_all, metrics_all)
